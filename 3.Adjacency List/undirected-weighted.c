...
struct node** graph_linking(struct node**,int,int,int);
struct node** linker(struct node**,int,int,int);
...
struct node** graph_linking(struct node** graph , int src , int dest,int weight){
   if (duplicate(graph[src]->next,src,dest))
   {
      return graph;
   }
    if(src == dest){   
        graph = linker(graph,src,dest,weight);
    }

    else{
         graph = linker(graph,src,dest,weight);
         graph = linker(graph,dest,src,weight);
    }
    return graph;
}

struct node** linker(struct node** graph,int src , int dest,int weight){
...
struct node** graph_construct(struct node** graph){
    int i = 0 , terminate = 0 , count = 0 , target = 0 , weight = 0;
    for(i = 0 ; i < v_num ; i++){
        ...
        while (terminate == 0 && count < v_num)
        {
            ...
            printf("\nEnter the weight:\n");
            scanf("%d",&weight);
            graph = graph_linking(graph,i,target-1,weight);
            count++;
        }
